...
import { buildResearchCompleteCard, formatProgressNotification, buildLaunchCardFeishu, buildLaunchText, buildProgressCardFeishu } from '../ui/cards.js';
...
export async function handleResearchCommand(context, topic) {
  ...
  const launchMsg = isFeishu(channel)
    ? buildLaunchCardFeishu(topic, modeInfo.name, reportUrl)
    : buildLaunchText(topic, modeInfo.name, reportUrl);
...
async function runBackgroundResearch(context, apiKey, topic, taskPath, conversationId, mode, taskId, startTime) {
  ...
  try {
    ...
    const { report } = await executeResearchStream({
      ...
      onProgress: async (progress) => {
        ...
        if (shouldNotify) {
          ...
          // 渠道感知：飞书用卡片，其他用文本
          const progressMsg = isFeishu(channel)
          ...
          await context.reply(progressMsg);
        }
    ...
    // 发送完成通知（渠道感知：飞书卡片，其他文本）
    const completeCard = buildResearchCompleteCard(topic, reportUrl, durationMinutes, mode, report, channel);
