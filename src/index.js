...
export default {
  ...
  async onMessageReceived(context) {
    ...
    // 0. Handle commands directly (since gateway command routing isn't working)
    if (text.startsWith('/')) {
      ...
      switch (command) {
        ...
        case 'key':
          return await handleKeyCommand(context, args);
        case 'ch':
          return context.reply(buildHelpMenu());
        default:
          break;
      }
    }
...
async function handleKeyCommand(context, args) {
  ...
  if (!args || args.length === 0) {
    const configuredKeys = [
      { name: 'CUECUE_API_KEY', key: getApiKeyFromSecrets('cuecue') },
      { name: 'TAVILY_API_KEY', key: getApiKeyFromSecrets('tavily') }
    ].filter(k => k.key);
